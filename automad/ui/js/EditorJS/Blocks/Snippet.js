...
class AutomadBlockSnippet {
	...
	constructor({ data, api }) {
		...
		this.wrapper.innerHTML = `
			...
			<hr>
			${
				create.label(AutomadEditorTranslation.get('snippet_input'), [
					'am-block-label',
			...
			${textarea.outerHTML}
			${
				create.label(AutomadEditorTranslation.get('snippet_file'), [
					'am-block-label',
				]).outerHTML
			}
			<div class="am-block-file-select uk-form-select uk-button uk-text-left uk-width-1-1" data-uk-form-select>
				...
				${
					create.select(
						['am-block-file'],
						['—'].concat(window.AutomadBlockTemplates.snippets),
						this.data.file
					).outerHTML
				}
			</div>
