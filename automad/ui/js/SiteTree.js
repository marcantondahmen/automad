	...
	Automad.SiteTree = {
		...
		urlAsValue: function ($tree, $input) {
			...
			if ($currentPageLink.length > 0) {
				$input.val(
					decodeURIComponent(
						$currentPageLink.attr('href').split('=')[1]
					)
				);
			}
			...
			$trees.each(function () {
				...
				$tree
					...
					.each(function () {
						...
						if (
							$node.hasClass(t.class.active) ||
							$node.find('.' + t.class.active).length > 0
						) {
						...
						$button.click(function (e) {
							...
							$children.toggle();
							$icon.toggleClass(
								t.class.iconOpen + ' ' + t.class.iconClose
							);
						});
					});
