	...
	Automad.Form = {
		...
		init: function () {
			...
			$doc.on('ajaxComplete', function (e, xhr, settings) {
				...
				$('button[' + da.submit + '="' + controller + '"]').prop(
					'disabled',
					true
				);
				...
				$('[' + da.controller + '="' + controller + '"]')
					.find('.' + f.unsavedClassInput)
					.removeClass(f.unsavedClassInput);
			});
			...
			var onFormChange = function () {
					...
					$(
						'button[' +
							...
							'="' +
							controller +
							'"]:disabled, .am-inpage [type="submit"]'
					).prop('disabled', false);

					// Change label color to flag input as changed.
					$(this)
						.closest('.uk-form-row, .am-inpage form')
						.find('.uk-form-label')
						.addClass(f.unsavedClassInput);
				},
				noAuto = `[${da.controller}]:not([${da.autoSubmit}])`,
				noExclude = `:not([${da.watchExclude}])`;

			$doc.on(
				'drop cut paste keydown',
				...
				'show.uk.modal.automad.form': function () {
					...
					if ($('html.uk-notouch').length) {
						...
						setTimeout(function () {
							$modal
								.find(
									'textarea, input:not(:disabled, [type="hidden"], [type="search"])'
								)
								.first()
								.focus();
						}, 500);
