...
+(function (Automad, $) {
	Automad.Upload = {
		...
		// Dropzone.
		$dropzone: $('<div></div>', {
			class: 'am-files-dropzone uk-hidden-touch',
		}),
		...
		init: function (e) {
			...
			u.$dropzone
				.html(
					iconDropzone +
						u.$modal.data(util.dataCamelCase(da.dropzoneText))
				)
			...
			u.$browse
				.html(
					iconBrowse +
						u.$modal.data(util.dataCamelCase(da.browseText))
				)
			...
			$(u.selectors.modal).on(
				'hide.uk.modal',
				{ $filesForm: u.$modal.closest('form') },
				u.refresh
			);
		},
			...
			u.$input.fileupload({
				...
				add: function (e, data) {
					...
					$.each(data.files, function (i) {
						info +=
							'<div class="am-text-white uk-text-truncate">' +
							data.files[i].name +
							'</div>' +
					...
					$(
						'<div class="uk-progress uk-progress-striped uk-active"><div class="uk-progress-bar"></div></div>'
				...
				stop: function (e) {
					...
					data.context
						.find('.uk-progress')
						.removeClass('uk-progress-striped uk-active');

					// In case of an server-side error, show error message in progress bar.
					if (data.result.error) {
						data.context
							.find('.uk-progress')
							.addClass('uk-progress-danger')
							.find('.uk-progress-bar')
							.text(data.result.error);
					} else {
						data.context
							.find('.uk-progress')
							.addClass('uk-progress-success');
					}
				},
