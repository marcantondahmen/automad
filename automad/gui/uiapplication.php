...
class UIApplication {
	...
	public function __construct() {
		...
		if (User::get()) {
			...
			// Check if context/controller matches an existing .php file.
			...
			} else if (Request::query('controller')) {

				header('Content-Type: application/json');
			
				$action = Request::query('controller');
				$output = call_user_func(__NAMESPACE__ . '\\Controllers\\' . $action);
		...
		$this->output = Prefix::tags($this->output);
