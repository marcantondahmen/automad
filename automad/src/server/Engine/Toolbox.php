...
class Toolbox {
	...
	public function breadcrumbs(array $options = array()): string {
		...
		foreach ($Selection->getSelection($options['excludeHidden']) as $Page) {
			$html .= '<li><a href="' . $Page->url . '">' . Str::stripTags($Page->get(Fields::TITLE)) . '</a></li> ';
		}
	...
	public function nav(array $options = array()): string {
		...
		foreach ($pages as $Page) {
			...
			$html .= '><a href="' . $Page->url . '">' . Str::stripTags($Page->get(Fields::TITLE)) . '</a></li>';
		}
		...
		if (isset($context)) {
			...
			if ($pages = $Selection->getSelection($options['excludeHidden'])) {
				...
				foreach ($pages as $Page) {
					...
					$html .= '><a href="' . $Page->url . '">' . Str::stripTags($Page->get(Fields::TITLE)) . '</a>';
