...
- **`php://input` is not broadly restricted on shared hosting — a note here previously claimed otherwise, which was a mistake, verified against the PHP manual.** `allow_url_fopen` has no effect on it at all; that setting only gates true remote wrappers (`http://`, `ftp://`, ...) used to fetch external URLs, a common misconception. `allow_url_include` (default `Off` on every PHP install) restricts using `php://input` as an `include()`/`require()` target, not plain reads via `file_get_contents()`. The one real, universal restriction: `php://input` is empty for `multipart/form-data` request bodies by default (`enable_post_data_reading` is on by default) — that's standard PHP behavior on every install, not something particular hosts disable.
- That multipart restriction is *why* `Automad\API\RequestHandler` (the dashboard's `/_api` mechanism) reads structured POST data via a `$_POST['__json__']` field instead of a raw body (`RequestHandler.php`: `$_POST = array_merge($_POST, json_decode($json, true))`): the dashboard client (`core/request.ts`) always sends requests as a `FormData` object — so plain data and file uploads share one code path — and a `FormData` body is always sent as `multipart/form-data`, for which `php://input` is genuinely empty. This is not a hosting-portability workaround; there's no reason to refactor it to read `php://input` instead, and doing so would break nothing except by wasting the effort.
- `automad/src/server/Controllers/McpController.php` reads `php://input` directly for its MCP JSON-RPC body, and that's correct: MCP clients send `application/json` (never multipart), so the body is fully readable there. This is required by the external MCP specification, not a convention of Automad's own — don't "fix" it back out.
...
- The MCP integration authenticates with a plain static Bearer token rather than OAuth — no authorize/token endpoints, no discovery metadata. Tokens are issued through the dashboard (`Controllers/API/McpTokenController::addToken()`, shown once, only a hash is persisted) and sent by the MCP client as a static `Authorization: Bearer <token>` header, the same pattern GitHub's MCP server and most personal-access-token-based integrations use. This was chosen for simplicity (a well-known, minimal auth pattern), independent of the `php://input` point above — don't reintroduce OAuth for this endpoint without a real reason to.
